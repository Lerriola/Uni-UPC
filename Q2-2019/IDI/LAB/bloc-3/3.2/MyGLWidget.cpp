...
void MyGLWidget::iniFocus ()
{
  ...
  posFocus = View * glm::vec4(1.0,1.0,1.0,1.0);  // en SCO
...
void MyGLWidget::actPosFocus()
{
  if(TypeFocus) posFocus = View * glm::vec4(X_f,1.0,1.0,1.0); 
  else posFocus = glm::vec4(0.0,0.0,0.0,1.0);
...
void MyGLWidget::normalizeTransform()
{
  // Matriu de normalitzacio d'objectes
  glm::mat3 normMat = glm::inverseTranspose(glm::mat3(View*TG));
  glUniformMatrix3fv (normMatLoc, 1, GL_FALSE, &normMat[0][0]);
}
...
void MyGLWidget::carregaShaders()
{
  ...
  normMatLoc = glGetUniformLocation (program->programId(),"normMat");
